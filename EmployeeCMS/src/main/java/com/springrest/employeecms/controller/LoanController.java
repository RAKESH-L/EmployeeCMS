...
import com.springrest.employeecms.repository.AccoutHolderLoanRepository;
...
public class LoanController {
	...
	@Autowired
	private AccoutHolderLoanRepository accountHolderLoanRepository;
	@PostMapping("/add")
	...
	public void applyLoan(@PathVariable("ahid") Long ahid, 
		...
		AccoutHolder ah =   accountHolderRepository.findById(ahid).get();
	...
	public List<AccoutHolder> getAHByLoan(@PathVariable("lid") Long lid) {
		Optional<Loan> optional = loanRepository.findById(lid);
		if(!optional.isPresent()) {
		...
		List<AccoutHolder> list =  accountHolderRepository.getAHByLoanId(lid);
		return list; 
	}
}
